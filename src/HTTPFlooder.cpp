...
void HTTPFlooder::work()
{
	...
	void HTTPFlooder::checkTimeOut()
	{
		while(flooding)
		{
			...
			std::unique_lock<std::mutex> lk(m_timeout);
	...
	void HTTPFlooder::Start()
	{
		...
		std::thread t(&loic::HTTPFlooder::checkTimeOut,this);
		t.detach();
		for(unsigned int i = 0; i<numThreads;i++)
		{
			std::thread th(&loic::HTTPFlooder::work,this);
			th.detach();
		}
	...
	{
		std::unique_lock<std::mutex> lk(m_stop);
		lk.lock();
		flooding = false;
		lk.unlock();
	}
